<?php
    ...
    elseif( is_post_type_archive('dt_classes') ):
		$title = __("Classes Archives",'dt_themes');
		dttheme_custom_subtitle_section( $title, "courses-bg");
    ...
    elseif( is_post_type_archive('dt_teachers') ):
		$title = sprintf(esc_html__('%s Archives', 'dt_themes'), $GLOBALS['teachers-singular-label']);
    ...
    elseif( is_single() ):
        if( is_attachment() ):
            ...
            if( $post_type === 'post' )   {
                ...
                dttheme_subtitle_section( $post->ID, 'dt_classes' );
