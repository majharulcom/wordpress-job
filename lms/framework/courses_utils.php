...
if($courses_type != 'popular') {
	...
	if( $wp_query->have_posts() ):  
		...
		echo '<div class="dt-sc-results-found">'.esc_html__('Records Found', 'dt_themes').' : <span>'.$wp_query->found_posts.'</span></div>';
				...
				<div class="dt-sc-course-thumb">
					<a href="<?php echo get_permalink($course_item_id); ?>" >
						...
						if(has_post_thumbnail($course_item_id)):
							...
							echo "<img src='".$img_attributes[0]."' width='".$img_attributes[1]."' height='".$img_attributes[2]."' />";
						elseif($pholder != 'on'): ?>
							<img src="http<?php echo dttheme_ssl(); ?>://placehold.it/1170x822&text=<?php echo get_the_title(); ?>" alt="<?php echo $course_item->post_title; ?>" />
						<?php endif; ?>
					 </a>
                    <div class="dt-sc-course-overlay">
