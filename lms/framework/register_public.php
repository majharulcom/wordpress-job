...
function plugin_head_styles_scripts() {
	...
	wp_enqueue_script('plugins-script', IAMD_FW_URL.'js/public/jsplugins.js',array(),false,true);
	...
	wp_localize_script('custom-script', 'DtCustomObject', array (
		'joinedGroup' => esc_html__('Joined Group Successfully', 'dt_themes'),
	));
...
if( !function_exists( 'get_events_title' ) ){
	function get_events_title() {
			...
			if( tribe_is_month() && !is_tax() ) {
				...
				$title = sprintf( __( 'Events for %s', 'dt_themes' ), date_i18n( 'F Y', strtotime( tribe_get_month_view_date() ) ) );
				
			} elseif( class_exists('Tribe__Events__Pro__Main') && tribe_is_week() ) {
				
				$title = sprintf( __('Events for week of %s', 'dt_themes'), date_i18n( $date_format, strtotime( tribe_get_first_week_day($wp_query->get('start_date') ) ) ) );
				
			} elseif( class_exists('Tribe__Events__Pro__Main') && tribe_is_day() ) {
				
				$title = __( 'Events for', 'dt_themes' ) . ' ' . date_i18n( $date_format, strtotime( $wp_query->get('start_date') ) );
				
			} elseif( class_exists('Tribe__Events__Pro__Main') && (tribe_is_map() || tribe_is_photo()) ) {
			
				if( tribe_is_past() ) {
					$title = __( 'Past Events', 'dt_themes' );
				} else {
