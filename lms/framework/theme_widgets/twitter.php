...
<?php
	...
	function widget($args, $instance) {
		...
		echo $before_widget;			
			...
			if ( !empty( $title ) ) { echo $before_title . '<a href="http'.dttheme_ssl().'://twitter.com/'.$username.'/" title="'.strip_tags($title).'">'.$title .'</a>'. $after_title; };
			...
			if($username && $consumer_key && $consumer_secret && $access_token && $access_token_secret && $count) { 
				...
				if($twitter && is_array($twitter)) {
					foreach( $twitter as $tweet ):
						...
						$latestTweet = preg_replace('/http:\/\/([a-z0-9_\.\-\+\&\!\#\~\/\,]+)/i', '<a href="http'.dttheme_ssl().'://$1" target="_blank">http://$1</a>', $latestTweet);
						$latestTweet = preg_replace('/@([a-z0-9_]+)/i', '<a href="http'.dttheme_ssl().'://twitter.com/$1" target="_blank">@$1</a>', $latestTweet);
						...
						echo '<li class="tweet">';
								...
								if( $display_avatar )
								echo '<span class="tweet-thumb"><a href="http'.dttheme_ssl().'://twitter.com/'.$username.'" title=""><img src="'.$tweet->user->profile_image_url.'" alt="" /></a></span>';
