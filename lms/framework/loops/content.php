			...
			?>
				<?php if( $format === "image" || empty($format) ): ?>
                        ...
                        <?php if( has_post_thumbnail() ):
                              ...
                              elseif($pholder != 'on'):?>
                                <img src="http<?php echo dttheme_ssl(); ?>://placehold.it/1170x822&text=<?php echo the_title(); ?>" alt="<?php printf(esc_attr__('%s'),the_title_attribute('echo=0'));?>" title="<?php printf(esc_attr__('%s'),the_title_attribute('echo=0'));?>" />
                        <?php endif;?>
                        </a>
                      ...
                      elseif( $format === "audio" && (array_key_exists('oembed-url', $post_meta) || array_key_exists('self-hosted-url', $post_meta)) ):
                        ...
                        <a href="<?php the_permalink();?>" title="<?php printf(esc_attr__('%s'),the_title_attribute('echo=0'));?>"><?php
                            if( has_post_thumbnail() ):
                                ...
                                echo "<img src='".$img_attributes[0]."' width='".$img_attributes[1]."' height='".$img_attributes[2]."' />";
                            elseif($pholder != 'on'):?>
                                <img src="http<?php echo dttheme_ssl(); ?>://placehold.it/1170x822&text=<?php echo the_title(); ?>" alt="<?php printf(esc_attr__('%s'),the_title_attribute('echo=0'));?>" title="<?php printf(esc_attr__('%s'),the_title_attribute('echo=0'));?>" />		<?php endif;?></a>                  
                <?php endif; ?>
            <?php } else { ?>
					<?php
