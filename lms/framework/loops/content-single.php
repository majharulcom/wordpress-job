	...
	<div class="entry-thumb">
		<?php if( ($format === "image" || empty($format)) && !array_key_exists("disable-featured-image", $tpl_default_settings) ): ?>
				...
				<?php if( has_post_thumbnail() ):
					  ...
					  elseif($pholder != "on"):?>
					  	<img src="http<?php echo dttheme_ssl(); ?>://placehold.it/1170x822&text=Image" alt="<?php printf(esc_attr__('%s'),the_title_attribute('echo=0'));?>" title="<?php printf(esc_attr__('%s'),the_title_attribute('echo=0'));?>" />
				<?php endif;?>
				</a>
			 ...
			 ?>
                  	<a href="<?php the_permalink();?>" title="<?php printf(esc_attr__('%s'),the_title_attribute('echo=0'));?>"><?php
						if( has_post_thumbnail() ):
							the_post_thumbnail("full");
						elseif($pholder != "on"):?>
                        	<img src="http<?php echo dttheme_ssl(); ?>://placehold.it/1170x822&text=Image" alt="<?php printf(esc_attr__('%s'),the_title_attribute('echo=0'));?>" title="<?php printf(esc_attr__('%s'),the_title_attribute('echo=0'));?>" />		<?php endif;?></a>                  
            <?php endif; ?>
	</div>
